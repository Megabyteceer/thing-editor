		...
		classesList.forEach((classInfo, i) => {
			...
			if(classPath.endsWith('.js')) {
				...
				if(wrongSymbolPos >= 0) {
					...
					scriptSource.push("import C" + i + " from '" + location.origin + '/' + classPath + "?v=" + (cacheCounter) + "'; editor.ClassesLoader.classLoaded(C" + i + ", '" + classPath + "', '" + (classInfo.lib || '') + "');");
				}
			}
		});
	
		let src = 'data:application/javascript,' + encodeURIComponent(scriptSource.join(';'));
	
		editor.ui.modal.showSpinner();
		let loadedFine;
		let d = import(src).then(() => { // eslint-disable-line
			if(!errorOccurred) {
				loadedFine = true;
				Lib._setClasses(classesById, classesDefaultsById);
			
				classesLoadedSuccessfullyAtLeastOnce = true;
				console.log(loadedClassesCount + ' custom components total loaded.');
				resolve();
